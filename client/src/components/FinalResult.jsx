...
const markDownComponent = {
     ...
     h3: ({children}) => (
      <h3 className='text-lg font-bold text-indigo-800 mt-4 mb-2'>
        {children}
      </h3>
     ),
  ...
  return (
    <div className='mt-6 p-3 space-y-10 bg-white'>
      ...
      <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
        ...
        <div className='flex gap-3'>
          <button onClick={()=>setQuickRevision(!quickRevision)} className={`
            ...
            }>{quickRevision ? "Exit Revision Mode": "Quick Revision (5min)"}</button>
          <button onClick={()=>downloadPdf(result)} className='px-4 py-2 rounded-lg text-sm font-medium bg-indigo-600
      ...
      {!quickRevision && <section>
        ...
        {quickRevision && 
        <section className='rounded-xl bg-gradient-to-r from-green-100 to-green-50
        border
        ...
        {result.diagram?.data && <section>
           ...
           <p className='mt-3 text-xs text-gray-500 italic'>
             ...
             your can save it by taking a screenshot.
           </p>
        </section>}
